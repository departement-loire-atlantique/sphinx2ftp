...
def sphinx_login(args):
  api_auth = args.sphinx_base_url + 'oauth/token'
  payload = {
  ...
  print "Sphinx authentication succeded"
  return auth['access_token']

def sphinx_download_survey(auth, args, localfilename='dataset.csv'):
  surveydata_url = args.sphinx_base_url + '/api/survey/' + args.sphinx_survey_id + '/data'
  ...
  try:
    ...
    raise Exception("sphinx_survey_id is incorrect or sphinx in unavailable : {}".format(e)) 
  
  print "Sphinx download survey succeded"
...
if __name__ == '__main__':
  ...
  parser.add_argument("--sphinx_base_url", default='https://asp-loire-atlantique.sphinxonline.net/sphinxapi/')
